...
while True:
    ...
    try:
        ...
        connection = connect(default_wifi,default_passw)
        if connection==None:
        ...
        for i in range(4):
            ...
            if connection!=None:
                break
            connection = connect(str(accesspoints[i][0],'UTF-8'),"")
        
        tft.text((0,row_index+20),"You choose "+used_wifi,TFT.GREEN,sysfont,1)
        tft.text((0,row_index+30),"Your IP address is "+used_ip,TFT.BLUE,sysfont,1)
        if connection !=None:
            serve(connection)
